...
while True:
    ...
    if len(classIds) != 0:
        for classId, confidence, box in zip(classIds.flatten(), confs.flatten(), bbox):
            ...
            if  beast_detected or danger_detected:
                if person_detected and beast_detected :
                    message = "Alert! Person and Beast Detected Together"
                    bot.send_message(chat_id, message)
                elif person_detected and danger_detected :
                    message = "Alert! Person and Danger Detected Together"
                    bot.send_message(chat_id, message)
                elif danger_detected :
                    message = "Alert! Danger Detected"
                    bot.send_message(chat_id, message)
                elif beast_detected :
            ...
            if  signs_detected or vehicle_detected:
                if person_detected and signs_detected :
                    message = "Alert! Person is spotted in proximity of Danger zone"
                    bot.send_message(chat_id, message)
                if vehicle_detected and signs_detected:
                    message = "Alert! Person Might be Tresspassing"
                    bot.send_message(chat_id, message)
                sendbot(img,message)


            if cutting_objects_detected:
                if person_detected and cutting_objects_detected  :
